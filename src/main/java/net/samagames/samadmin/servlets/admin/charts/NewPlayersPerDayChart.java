 ...
 */
public class NewPlayersPerDayChart extends Chart
{
    public NewPlayersPerDayChart(SamAdmin instance)
    {
        super("Nouveaux joueurs (Mois)");
        ...
        ChartData entry = this.addEntry("spline", "Nouveaux joueurs (" + total + ")", false, true);
    ...
    public HashMap<Integer, Integer> getNewPlayersPerDay(SamAdmin instance, Calendar calendar)
    {
        ...
        {
            PreparedStatement pStatement = sql.prepareStatement("SELECT * FROM sg_players WHERE firstLogin LIKE '" + calendar.get(Calendar.YEAR) + "-" + (calendar.get(Calendar.MONTH) + 1) + "%'");

            ResultSet resultSet = pStatement.executeQuery();

            while (resultSet.next())
            {
                int day = resultSet.getTimestamp("firstLogin").getDate();
